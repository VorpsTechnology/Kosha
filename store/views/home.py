...
def customer_address_view(request):
    ...
    if 'ids' in request.session and request.session['ids']:
        product_in_cart = True

    product_count_in_cart = len(set(request.session.get('ids', '').split('|'))) if 'ids' in request.session else 0
    ...
    if request.method == 'POST':
        ...
        if addressForm.is_valid():
            ...
            total = 0
            if 'ids' in request.session and request.session['ids']:
                product_id_in_cart = request.session['ids'].split('|')
...
#         productids = request.session['ids']
#         if productids != "":
#             product_in_cart = True

#     if 'ids' in request.session:
#         productids = request.session['ids']
#         counter = productids.split('|')
...
#                 productids = request.session['ids']
#                 if productids != "":
#                     product_id_in_cart = productids.split('|')
#                     products = Products.objects.filter(id__in=product_id_in_cart)
#                     for p in products:
#                         total += p.price
...
def payment_success_view(request):
    ...
    # customer = Customer.objects.get(user=request.email)
    products = None
    email = None
    phone = None
    address = None
    ...
    # Place an order for each product
    product_ids = request.session.get('ids')

    if product_ids:
        product_id_in_cart = product_ids.split('|')

        # Place an order for each product
        for product_id in product_id_in_cart:
